...
async function scan(dir: string, packages: PackageInfo[]): Promise<void> {
  ...
  if (manifest) {
    ...
    const hasPython = entries.some((entry) => ["setup.py", "setup.cfg", "pyproject.toml"].includes(entry.name));
    packages.push({
  ...
  for (const entry of entries) {
    if (entry.isDirectory() && !entry.name.startsWith(".")) await scan(join(dir, entry.name), packages);
  }
}

export async function inspectWorkspace(cwd: string): Promise<WorkspaceInfo> {
  const root = await findWorkspace(cwd);
  if (!root) return { hasSrc: false, hasBuild: false, hasInstall: false, hasLog: false, packages: [], duplicateNames: [] };
