export interface PackageScaffoldInput {
  packageName: string;
  language: 'python' | 'cpp';
  nodeName: string;
}
export function packageScaffold(input: PackageScaffoldInput) {
  if (!/^[a-z][a-z0-9_]*$/.test(input.packageName))
    throw new Error('packageName must use lowercase letters, digits, and underscores');
  if (!/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(input.nodeName))
    throw new Error('nodeName must be a valid identifier');
  const py = input.language === 'python';
  const files: Record<string, string> = {};
  files[`${input.packageName}/package.xml`] =
  ...
  if (py) {
    files[`${input.packageName}/setup.py`] =
    ...
    files[`${input.packageName}/${input.packageName}/${input.nodeName}.py`] =
      `import rclpy\nfrom rclpy.node import Node\n\nclass ${input.nodeName}(Node):\n    def __init__(self):\n        super().__init__('${input.nodeName}')\n\ndef main():\n    rclpy.init()\n    node = ${input.nodeName}()\n    rclpy.spin(node)\n    node.destroy_node()\n    rclpy.shutdown()\n`;
  } else {
    files[`${input.packageName}/CMakeLists.txt`] =
    ...
    files[`${input.packageName}/src/${input.nodeName}.cpp`] =
