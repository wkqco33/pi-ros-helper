...
export async function snapshotGraph(cwd: string, signal?: AbortSignal): Promise<GraphSnapshot> {
  const [nodes, topics, services] = await Promise.all([
    ...
    runCommand("ros2", ["topic", "list", "-t"], { cwd, signal, timeoutMs: 5000, maxBytes: 100_000 }),
  ...
  if (nodes.code !== 0 && topics.code !== 0 && services.code !== 0) throw new Error(topics.stderr || nodes.stderr || services.stderr || "ROS graph is unavailable");
  return { nodes: lines(nodes.stdout), topics: parsedTopics, services: lines(services.stdout), capturedAt: new Date().toISOString(), source: "ros2-cli" };
}
...
export function diffGraphs(previous: GraphSnapshot, current: GraphSnapshot) {
  ...
  return {
    nodes: { added: added(current.nodes, previous.nodes), removed: removed(current.nodes, previous.nodes) },
    topics: { added: added(currentTopics, prevTopics), removed: removed(currentTopics, prevTopics) },
    services: { added: added(current.services, previous.services), removed: removed(current.services, previous.services) },
  };
}
