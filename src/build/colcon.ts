...
async function collectXml(dir: string, output: TestResult[]): Promise<void> {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) await collectXml(path, output);
      ...
      const failures = Number(xml.match(/failures="(\d+)"/)?.[1] ?? 0) + Number(xml.match(/errors="(\d+)"/)?.[1] ?? 0);
      const errors = [...xml.matchAll(/<(?:failure|error)[^>]*>([\s\S]*?)<\/(?:failure|error)>/g)].map((m) => m[1].replace(/<[^>]+>/g, "").trim()).filter(Boolean).slice(0, 5);
