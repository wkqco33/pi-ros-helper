...
test('graph diff is order independent', () => {
  const previous = {
    nodes: ['/a'],
    topics: [{ name: '/old', types: ['x'] }],
    services: [],
    capturedAt: 'a',
    source: 'ros2-cli' as const,
  };
  const current = {
    nodes: ['/a', '/b'],
    topics: [{ name: '/new', types: ['x'] }],
    services: ['/service'],
    capturedAt: 'b',
    source: 'ros2-cli' as const,
  };
  assert.deepEqual(diffGraphs(previous, current), {
    nodes: { added: ['/b'], removed: [] },
    topics: { added: ['/new'], removed: ['/old'] },
    services: { added: ['/service'], removed: [] },
  });
});
...
test('package scaffold validates names and creates required files', () => {
  const generated = packageScaffold({
  ...
  assert.throws(() =>
    packageScaffold({ packageName: 'Bad-Package', nodeName: 'demo', language: 'cpp' }),
  );
});
